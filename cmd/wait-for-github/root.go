...
import (
	...
	"github.com/grafana/wait-for-github/internal/logger"
	"github.com/urfave/cli/v2"
)

type cmdFunc func(cfg *config) *cli.Command

var defaultLogLevel logger.Level = logger.Level(slog.LevelInfo)
...
func handleGlobalConfig(c *cli.Context) (config, error) {
	...
	_ = logger.SetupLogger(slog.Level(defaultLogLevel))
	cfg.logger = slog.Default()

	cfg.logger.DebugContext(c.Context, "debug logging enabled")
	...
	if token != "" {
		cfg.logger.DebugContext(c.Context, "will use github token for authentication")
		cfg.logger.DebugContext(c.Context, "using token starting with", "token", token[:10])
	...
	if len(privateKey) == 0 || appId == 0 || installationID == 0 {
		cfg.logger.ErrorContext(c.Context, "must provide either a GitHub token or a GitHub app private key, app ID and installation ID")
